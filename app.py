...
def get_grade(score):
    if score <= 30:   return "주의","[빨강]","#FF4B4B"
    elif score <= 70: return "보통","[노랑]","#FFA500"
    else:             return "충족","[파랑]","#1E90FF"
...
with st.sidebar:
    ...
    for pname, pdesc in pages:
        ...
        bord = "rgba(124,111,255,0.3)"  if is_active else "transparent"
        st.markdown(
            f"<div style='background:{bg};border-radius:10px;padding:10px 14px;margin-bottom:2px;"
            f"border:1px solid {bord};'>"
            f"<span style='color:{col};font-weight:{'700' if is_active else '400'};font-size:14px;'>{pname}</span><br>"
...
if page == "대시보드":
    ...
    if not info_complete:
        st.info("왼쪽 메뉴에서 프로필 & 목표를 먼저 설정해주세요!")
            ...
            if model_loaded:
                if (st.session_state.last_pred is None or
                    ...
                    with st.spinner("AI 분석 중..."):
                        ...
                        info = food_info[key]
                        st.session_state.last_pred = {"filename": uploaded_file.name, "info": info, "conf": conf}
                else:
...
# ===== 하루 평가 =====
    ...
    if not info_complete:
        st.warning("먼저 프로필 & 목표에서 정보를 입력해주세요.")
    elif not st.session_state.foods:
        st.warning("등록된 식사가 없습니다. 음식 등록에서 식사를 추가해주세요.")
    else:
