...
[data-testid="stSelectbox"] > div > div:focus-within, [data-testid="stNumberInput"] > div > div > input:focus { border-color: #7c6fff !important; box-shadow: 0 0 0 2px rgba(124,111,255,0.2) !important; }
...
defaults = {
    ...
    "last_pred": None,        # 마지막 AI 예측 결과 저장
    "upload_reset_key": 0,    # 파일 업로더 리셋용 키
}
...
def grade_badge_html(score, label=""):
    g, icon, color = get_grade(score)
    prefix = f"{label} " if label else ""
    return (f"<div style='display:inline-block;padding:7px 20px;border-radius:20px;"
            f"background:{color}22;border:2px solid {color};color:{color};"
            f"font-weight:700;font-size:15px;'>{icon} {prefix}{g} &nbsp;|&nbsp; {score}점</div>")
...
with st.sidebar:
    ...
    st.markdown("<div style='font-size:10px;color:#8080a0;letter-spacing:1.5px;text-transform:uppercase;padding:0 4px 8px;'>메뉴</div>", unsafe_allow_html=True)
    ...
    for pname, pdesc in pages:
        is_active = st.session_state.page == pname
        col = "#7c6fff" if is_active else "#8080a0"
        bg  = "rgba(124,111,255,0.15)" if is_active else "transparent"
        fw  = "700" if is_active else "400"
        st.markdown(
            f"<div style='background:{bg};border-radius:10px;padding:10px 14px;margin-bottom:2px;"
            f"cursor:pointer;border:1px solid {'rgba(124,111,255,0.3)' if is_active else 'transparent'};'>"
            f"<span style='color:{col};font-weight:{fw};font-size:14px;'>{pname}</span><br>"
            f"<span style='color:#555;font-size:11px;'>{pdesc}</span></div>",
            unsafe_allow_html=True
        )
        if st.button(pname, key=f"nav_{pname}", help=pdesc):
    ...
    if st.session_state.night_count >= 3:
        st.error(f"🚨 야식 {st.session_state.night_count}회!\n야식을 줄여주세요.")

    st.markdown("<hr style='border-color:rgba(255,255,255,0.06);margin:12px 0;'>", unsafe_allow_html=True)
    if st.button("🔄 데이터 초기화", key="reset_btn"):
        page_bk = st.session_state.page
        st.session_state.clear()
        st.session_state.page = page_bk
        st.rerun()
...
# ============================================================
if page == "🏠 대시보드":
    st.markdown("<h1>🏠 오늘의 식단 현황</h1>", unsafe_allow_html=True)
    st.markdown("<p style='color:#8080a0;margin-bottom:24px;'>오늘 하루 섭취 현황을 한눈에 확인하세요.</p>", unsafe_allow_html=True)
    ...
    if not info_complete:
        st.info("👈 왼쪽 메뉴에서 **프로필 & 목표**를 먼저 설정해주세요!")
    ...
    if st.session_state.foods:
        ...
        for meal_type, food_name in st.session_state.foods:
            ...
            st.markdown(
                ...
                f"<span style='font-size:14px;'>{food_name}</span>"
        ...
        st.markdown(
            ...
            "📸 아직 등록된 식사가 없습니다.<br><small>왼쪽 메뉴 → 음식 등록에서 추가하세요.</small></div>",
            unsafe_allow_html=True
        )
...
# ============================================================
    ...
    st.markdown("<h1>👤 프로필 설정</h1>", unsafe_allow_html=True)
    st.markdown("<p style='color:#8080a0;margin-bottom:24px;'>정보를 입력하면 BMR 기반으로 맞춤 목표가 자동 계산됩니다.</p>", unsafe_allow_html=True)

    col1, col2 = st.columns(2)
    with col1:
        # ✅ 기본값 없이 None으로 시작
        gender = st.selectbox("성별", ["남","여"], index=None, placeholder="선택하세요")
        age    = st.number_input("나이 (세)", min_value=1, max_value=100,
                                  value=None, placeholder="입력하세요")
    with col2:
    ...
    goal = st.selectbox("목표", ["다이어트 (체중 감량)","유지 (체중 유지)","벌크업 (근육 증가)"],
                        index=None, placeholder="선택하세요")

    if st.button("✓ 목표 계산하기"):
        if None in (gender, age, height, weight, goal):
            ...
            target_calorie = tc; target_carb = tcarb
            target_protein = tp; target_fat  = tf_
            info_complete  = True
...
# ============================================================
    ...
    st.markdown("<h1>📸 음식 등록</h1>", unsafe_allow_html=True)
    st.markdown("<p style='color:#8080a0;margin-bottom:24px;'>사진을 업로드하면 AI가 15가지 음식을 자동 인식합니다.</p>", unsafe_allow_html=True)

    # ✅ 식사 종류 — 세션에서 관리 (페이지 이동해도 안 날아가게, 등록 후 리셋)
    meal_options = ["아침","점심","저녁","야식"]
    if "meal_type_idx" not in st.session_state:
    ...
    if uploaded_file is not None:
        ...
        with col_res:
            if model_loaded:
                # 이미 예측한 파일과 같으면 재사용, 새 파일이면 새로 예측
                ...
                if st.button("🍽️ 이 식사 등록하기"):
                    ...
                    # 리셋
                    st.session_state.upload_reset_key += 1  # 파일 업로더 초기화
                    st.session_state.meal_type_idx = 0      # 식사종류 아침으로 리셋
                    st.session_state.last_pred = None       # 예측 결과 초기화
...
# ============================================================
    ...
    st.markdown("<h1>✨ 영양소 추천</h1>", unsafe_allow_html=True)
    st.markdown("<p style='color:#8080a0;margin-bottom:24px;'>현재 식단에서 가장 부족한 영양소를 채울 음식 3가지를 추천합니다.</p>", unsafe_allow_html=True)

    if not st.session_state.foods:
        st.info("📸 음식을 먼저 등록해주세요.")
    elif not info_complete:
        st.info("👤 프로필 & 목표를 먼저 설정해주세요.")
    else:
        ...
        if kcal_ratio > 1.1:
            st.warning(f"⚠️ 칼로리를 {round(st.session_state.total_kcal - target_calorie)}kcal 초과했습니다! 오늘은 가벼운 식사를 권장합니다.")

        candidates = []
        if pr <= cr and pr <= fr:
            lack = "단백질"
            for f,fi in food_info.items(): candidates.append((fi["protein"]/max(fi["kcal"],1), fi))
        elif cr <= fr:
            lack = "탄수화물"
            for f,fi in food_info.items(): candidates.append((fi["carb"]/max(fi["kcal"],1), fi))
        else:
            lack = "지방"
            for f,fi in food_info.items(): candidates.append((fi["fat"]/max(fi["kcal"],1), fi))
...
# ============================================================
    ...
    st.markdown("<h1>💯 하루 평가</h1>", unsafe_allow_html=True)
    st.markdown(
        "<div style='background:#1a1a24;border-radius:10px;padding:12px 16px;margin-bottom:20px;"
        "display:flex;gap:24px;font-size:13px;'>"
        "<span>🔴 <b>주의</b> 0~30점</span><span>🟡 <b>보통</b> 31~70점</span><span>🔵 <b>충족</b> 71~100점</span>"
        "</div>", unsafe_allow_html=True
    )

    if not info_complete:
        st.warning("먼저 '프로필 & 목표'에서 정보를 입력해주세요.")
    elif not st.session_state.foods:
        st.warning("등록된 식사가 없습니다. '음식 등록'에서 식사를 추가해주세요.")
    else:
        c1,c2,c3,c4 = st.columns(4)
        c1.metric("섭취 칼로리", f"{st.session_state.total_kcal} kcal",
        ...
        if st.button("💯 오늘 하루 평가하기"):
            ...
            st.markdown(
                ...
                f"<div style='font-size:20px;font-weight:700;color:{color};margin-top:8px;'>{icon} {g}</div>"
                f"</div>", unsafe_allow_html=True
            )
            ...
            for idx,(item,d) in enumerate(details.items()):
                with cols[idx]:
                    ...
                    st.markdown(
                        ...
                        f"<div style='font-size:18px;font-weight:700;color:{c_};'>{i_} {d['status']}</div>"
...
# ============================================================
    ...
    st.markdown("<h1>📅 주간 리포트</h1>", unsafe_allow_html=True)
    ...
    if not st.session_state.weekly_scores:
            ...
            for i in [d-1 for d in caution_days]:
                if i < len(dlist):
                    for item,d in dlist[i].items():
                        if d["status"]=="주의": all_lack[item] = all_lack.get(item,0)+1
            if all_lack:
        ...
        bc = [get_grade(s)[2] for s in scores]

        for ax, data, title, ylabel in [(ax1,cals,"일별 칼로리","kcal"),(ax2,scores,"일별 평가 점수","점수")]:
            ...
            ax.set_xticks(days); ax.set_xticklabels([f"{d}일" for d in days])
            ax.set_xlabel("Day",color="#8080a0"); ax.set_ylabel(ylabel,color="#8080a0")
            ax.set_title(title, color="#f0f0f8", fontweight="bold")
            ax.tick_params(colors="#8080a0"); ax.spines[:].set_color("#333")
